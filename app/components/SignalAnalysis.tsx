...
import SaveButton from './SaveButton'; // Reuse the save button for retention
...
export default function SignalAnalysis({ studioName, context, method, tags, images }: SignalAnalysisProps) {
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 min-h-screen bg-primary-bg border-t border-secondary-bg">
      ...
      {/* LEFT PANE: Visual Evidence (Sticky Scroll on Desktop) */}
        ...
        <div className="flex flex-col">
          {images.map((img, i) => (
            <div key={i} className="relative w-full aspect-[4/5] md:aspect-auto md:h-screen group">
              ...
              />
              {/* Save Button on every image */}
              <SaveButton />
              <div className="absolute bottom-4 left-4 text-primary-bg/50 font-sans-body text-[10px] uppercase tracking-widest">
                Fig. {i + 1} — Evidence
              </div>
            </div>
          ))}
