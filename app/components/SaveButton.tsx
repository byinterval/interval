...
export default function SaveButton() {
  ...
  return (
    <button 
      onClick={(e) => {
        e.preventDefault(); // Prevents clicking the card link behind the button
        setSaved(!saved);
      }}
      className="absolute top-4 right-4 z-30 p-2 rounded-full transition-all duration-300 group/btn"
      aria-label="Save to Atlas"
    >
      {saved ? (
        // Active State: Filled Star (Accent Brown)
        ...
        // Default State: Empty Star (Visible on dark & light images)
